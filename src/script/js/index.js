...
// cooKie登录和退出
! function($) {
    var span = $(".bar_left span").eq(1);
    var a = span.children("a");

    if ($.cookie("user")) {
        // span.text("欢迎 "+$.cookie("user"));
        span.get(0).innerText = "欢迎 " + $.cookie("user");
    }
...
// 搜索框
! function($) {
    var input = $("#search_sale");
    var $search = $(".input_tips ")
    var outarray = [];
    var output = "";
    // 39   Right（右箭头）
    // 40  Down（下箭头）
    ...
    input.on("input", function() {
        ...
        url = "https://www.yohobuy.com/product/search/suggest?keyword=" + str;
        $.ajax({
            async: false,
            type: "get",
            "url": url,
            ...
            success: function(msg) {
                $.each(msg.data, function(i, n) {
                    outarray.push(n.keyword);

                })

                $.each(outarray.slice(0, 8), function(i, n) {
                    output += "<li>" + n + "</li>"
                });
                $search.children("ul").html(output);
                outarray = [];
